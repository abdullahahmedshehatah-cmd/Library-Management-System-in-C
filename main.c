...
// Define the filename for storing book data (now just for reference)
...
int main() {
    ...
    // Load book data using the new hardcoded version
    if (load_books(DATA_FILE, library, &book_count)) {
        printf("Starting with hardcoded books. Total books: %d\n", book_count);
    } else {
        printf("Error loading hardcoded books. Starting with an empty library.\n");
    }
    
    // We will now call admin_mode with the books we've loaded
    // and the code will work exactly as before, just without saving.
